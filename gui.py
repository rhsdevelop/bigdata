...
class Widgets():
    ...
    def button(self, field, cmd, textwidth, height, row, col, colspan=0):
        ''' Inserir button em formulários. '''
        text = tkinter.Button(self.instance, text=field, width=textwidth, command=cmd)
        if colspan:
    ...
    def label(self, field, textwidth, row, col, colspan=1, rowspan=1, fg='', height=0, stick=None):
        ''' Inserir texto livre em formulários. '''
        label = tkinter.Label(self.instance, text=field, width=textwidth)
        if self.color:
            label['bg'] = self.color
        if height:
            label['height'] = height
    ...
    def textbox(self, field, textwidth, row, col, default='', show='', cmd=''):
        ''' Inserir textbox em formulários. '''
        label = tkinter.Label(self.instance, text=field)
        text = tkinter.Entry(self.instance, width=textwidth)
        if self.color:
            label['bg'] = self.color
        if show:
            text['show'] = show
    ...
    def calendar(self, row, col, default='', rowspan=1, columnspan=1, command=False):
    ...
    def check(self, field, textwidth, checktext, row, col, selected=False, seek=''):
        ''' Inserir checkbox em formulários. '''
        label = tkinter.Label(self.instance, text=field)
        var = tkinter.IntVar()
        checkbox = tkinter.Checkbutton(self.instance, text=checktext,
                                       width=textwidth, variable=var)
        # var = True
        if self.color:
            label['bg'] = self.color
    ...
    def combobox(self, field, textwidth, combolist, row, col, default='', cmd='', seek=''):
        ''' Inserir combobox em formulários. '''
        label = tkinter.Label(self.instance, text=field)
        text = ttk.Combobox(self.instance, values=combolist, width=textwidth)
        if self.color:
            label['bg'] = self.color
        if cmd:
    ...
    def listbox(self, field, textwidth, height, combolist, row, col, cmd=None):
        ''' Inserir combobox em formulários. '''
        label = tkinter.Label(self.instance, text=field)
        text = tkinter.Listbox(self.instance, height=height, width=textwidth)
    ...
    def progressbar(self, length, row, col, colspam=1, rowspan=1, bg=''):
        ''' Inserir progressbar em formulários. leght padrão 200 '''
        progress = ttk.Progressbar(self.instance, orient="horizontal", length=length, mode="determinate")
        progress.grid(row=row, column=col, stick='E')
        return progress
    ...
    def menu(self, menus=[], opcoes=[]):
        ...
        for row in opcoes:
            if 'command' in row.keys():
                menu[row['root']].add_command(label=row['title'], command=row['command'])
            else:
                if 'root' in row.keys():
                    menu[row['root']].add_cascade(label=row['title'], menu=menu[row['menu']])
                else:
                    main_menu.add_cascade(label=row['title'], menu=menu[row['menu']])
